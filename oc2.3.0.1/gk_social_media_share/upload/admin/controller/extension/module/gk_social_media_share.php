 ...
 * www.goodkoding.com
 ****/
class ControllerExtensionModuleGkSocialMediaShare extends Controller {
    private $error = array();  
    ...
    public function install() {
        ...
        $this->model_extension_event->addEvent('gk_social_media_share', 'admin/model/catalog/product/addProduct/after', 'extension/module/gk_social_media_share/gk_social_media_share_add');
        $this->model_extension_event->addEvent('gk_social_media_share', 'admin/model/catalog/product/editProduct/after', 'extension/module/gk_social_media_share/gk_social_media_share_edit');
    }
	...
	public function index() {  
        $this->load->language('extension/module/gk_social_media_share');
		...
		$social_data = false;
		if(isset($gk_social_media_share['gk_social_media_share'])) {
        ...
        if(!empty($social_data['facebook']['app_id']) && !empty($social_data['facebook']['app_secret'])){
            ...
            if(!empty($social_data['facebook']['long_access_token'])) {
                $me = $facebook->get('/me', $social_data['facebook']['long_access_token']);
                $social_data['facebook']['user'] = $me->getDecodedBody()['id'];
            }

			if (isset($social_data['facebook']['user']) && $social_data['facebook']['user'] > 0) {
				try {
                    if(!$social_data || empty($social_data['facebook']['page_id'])) {
                        if(isset($_GET['page_id'])) {
                            ...
                            if(!isset($_GET['page_access_token'])) {
                                $social_data['facebook']['page_access_token'] = $facebook->get("/".$social_data['facebook']['page_id']."?fields=access_token", $social_data['facebook']['long_access_token']);
                            } else {                            
                            ...
                            $response = $facebook->get("/".$social_data['facebook']['user']."/accounts", $social_data['facebook']['long_access_token']);
				...
				$data['fb_login_url'] = $helper->getLoginUrl(HTTP_CATALOG . 'gk_social_media_share_redirect.php', $permissions);
			 }
        } else {
            $data['error_warning'] = $this->language->get('fb_authenticate_error');
        }
		...
		$data['entry_app_id_name'] = $this->language->get('entry_app_id_name');
		$data['entry_app_secret_name'] = $this->language->get('entry_app_secret_name');
        ...
        $data['button_remove'] = $this->language->get('button_remove');
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/gk_social_media_share', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$data['fb_logout_url'] = $this->url->link('extension/module/gk_social_media_share/logout', 'token=' . $this->session->data['token'], 'SSL');
        
		$data['action'] = $this->url->link('extension/module/gk_social_media_share', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('extension/module/gk_social_media_share.tpl', $data));
    }
    ...
    public function logout() {
		...
		$social_data['facebook']['long_access_token'] = '';
		$social_data['facebook']['page_access_token'] = '';
		$social_data['facebook']['page_id'] = '';
		$social_data['facebook']['page_name'] = '';
		$this->model_setting_setting->editSetting('gk_social_media_share',array('gk_social_media_share' => base64_encode(serialize($social_data))));
        $this->response->redirect($this->url->link('extension/module/gk_social_media_share', 'token=' . $this->session->data['token'], 'SSL'));
    }
    
    private function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/gk_social_media_share')) {
    ...
    public function gk_social_media_share_add($route, $args, $product_id = null) {
    ...
    public function gk_social_media_share_edit($route, $args) {

        $product_id = reset($args);
		...
		if(isset($share_fb_enable) && $share_fb_enable == 1 ){
			...
			if(!empty($social_data['facebook']['page_id']) && !empty($social_data['facebook']['page_access_token'])) { 
				...
				try {
					...
					$post_response = $facebook->post("/$page_id/feed", $args, $social_data['facebook']['page_access_token']);
					$post_id = $post_response->getDecodedBody()['id'];
